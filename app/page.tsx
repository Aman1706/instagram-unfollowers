...
const Home = () => {
  ...
  return (
    <main className="max-w-215 mx-auto px-7 pt-18 pb-35">
      ...
      <div className="my-5 h-px bg-linear-to-r from-edge to-transparent animate-rise [animation-delay:220ms]" />
      ...
      <div className="flex flex-col gap-5 md:flex-row items-start md:items-center justify-between mb-5 animate-rise [animation-delay:280ms]">
        ...
        <Link
          href="/how-to-export"
          className="text-[12px] tracking-[0.15em] uppercase text-rose no-underline transition-colors duration-150"
        >
          How to export ↗
        </Link>
      </div>
      ...
      <div className="mt-7 animate-rise [animation-delay:420ms] text-center sm:text-left">
        <button
          className="inline-flex items-center gap-3.5 py-3.75 px-9.5 bg-rose text-canvas rounded-sm font-mono text-[12px] tracking-[0.18em] uppercase font-medium cursor-pointer transition-[opacity,transform] duration-200 hover:opacity-85 hover:-translate-y-0.5 disabled:!opacity-[0.18] disabled:cursor-not-allowed"
          disabled={!canAnalyze}
      ...
      {analyzed && unfollowers !== null && (
        <section ref={resultsRef} className="mt-18 animate-rise scroll-mt-6">
          <div className="flex items-end gap-4.5 mb-9 pb-6 border-b border-edge">
            ...
            <div className="pb-1.5">
              ...
              <p className="text-[12px] text-dim tracking-widest uppercase">
          ...
          {unfollowers.length === 0 ? (
            ...
            <div className="flex flex-col">
              {unfollowers.map((user, i) => (
                <div
                  key={user.username}
                  className="flex items-center gap-4 py-3.25 border-b border-edge transition-[padding-left] duration-200 hover:pl-1.5"
                >
          ...
          <div className="mt-10 pt-6 border-t border-edge flex items-center gap-4">
            <button
              ...
              className="inline-flex items-center gap-3 py-3.25 px-7 border border-edge text-dim rounded-sm font-mono text-[12px] tracking-[0.18em] uppercase font-medium cursor-pointer transition-colors duration-200 hover:border-rose/40 hover:text-rose"
            >
              <span>↺</span>
              <span>Clear &amp; start over</span>
            </button>
            <span className="text-[12px] text-dim tracking-widest uppercase">
